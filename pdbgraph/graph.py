...
def construct_torch_graph(atoms, bonds):
    ...
    edge_index = []
    for bond in bonds:
        atom1_index = next(i for i, atom_info in enumerate(atoms) if tuple(atom_info['coord']) == tuple(bond['atom1_coord']))
        atom2_index = next(i for i, atom_info in enumerate(atoms) if tuple(atom_info['coord']) == tuple(bond['atom2_coord']))
